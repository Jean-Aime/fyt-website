...
if ($_POST) {
    if (verifyCSRFToken($_POST['csrf_token'])) {
        try {
            ...
            // Handle file uploads
            $featured_image = $is_edit ? $tour['featured_image'] : null;
            $gallery = $is_edit ? json_decode($tour['gallery'], true) : [];
            $brochure_pdf = $is_edit ? $tour['brochure_pdf'] : null;
            ...
            if ($is_edit) {
                ...
                $stmt = $db->prepare("
                    INSERT INTO tours (
                        ...
                        currency, includes, excludes, requirements,
                        cancellation_policy, featured_image, gallery,
                        brochure_pdf, status, featured, created_by
                    ) VALUES (
                        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
                    )
                ");
                ...
                $stmt->execute([
                    ...
                    $currency, $includes, $excludes, $requirements,
                    $cancellation_policy, $featured_image, json_encode($gallery),
                    $brochure_pdf, $status, $featured, $_SESSION['user_id']
                ]);
                
                $tour_id = $db->lastInsertId();
                $auth->logActivity($_SESSION['user_id'], 'tour_created', "Created new tour: $title");
                
                header("Location: edit.php?id=$tour_id&success=Tour created successfully!");
                exit;
            }
            
        } catch (Exception $e) {
            $error = 'Error saving tour: ' . $e->getMessage();
        }
    } else {
            ...
            <div class="content">
                <div class="content-header">
                    <div class="content-title">
                        ...
                        <p><?php echo $is_edit ? 'Update tour information and settings' : 'Create a new tour package with detailed information'; ?></p>
                    </div>
                            ...
                            <div class="form-section">
                                ...
                                <div class="form-grid">
                                    ...
                                    <div class="form-group full-width">
                                        ...
                                        <div class="form-help">URL-friendly version of the title (auto-generated)</div>
                                    </div>
...
<div class="form-group">
        ...
        <!-- This will be dynamically filled by JS -->
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="difficulty_level" name="difficulty_level" class="form-control">
                                            ...
                                            <option value="moderate" <?php echo ($tour['difficulty_level'] ?? 'moderate') === 'moderate' ? 'selected' : ''; ?>>Moderate</option>
                                    ...
                                    <div class="duration-grid">
                                        <div class="form-group">
                                            ...
                                            <input type="number" id="duration_days" name="duration_days" class="form-control" 
                                                   min="1" max="365" required value="<?php echo $tour['duration_days'] ?? 1; ?>">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" id="duration_nights" name="duration_nights" class="form-control" 
                                                   min="0" max="364" value="<?php echo $tour['duration_nights'] ?? 0; ?>">
                                        </div>
                                    </div>
                                ...
                                <div class="form-grid">
                                    <div class="form-group">
                                        ...
                                        <select id="currency" name="currency" class="form-control">
                                            <option value="USD" <?php echo ($tour['currency'] ?? 'USD') === 'USD' ? 'selected' : ''; ?>>USD ($)</option>
                            ...
                            <div class="form-section">
                                ...
                                <div class="form-grid">
                                    <div class="form-group">
                                        ...
                                        <div class="image-upload-area <?php echo ($tour['featured_image'] ?? '') ? 'has-image' : ''; ?>" 
                                            ...
                                            <?php if ($tour['featured_image'] ?? ''): ?>
                                                ...
                                                <div class="upload-text">Click to change featured image</div>
                                            <?php else: ?>
                                                <div class="upload-icon">
                                                    <i class="fas fa-cloud-upload-alt"></i>
                                                </div>
                                                <div class="upload-text">Click to upload featured image</div>
                                                <div class="form-help">Recommended: 1200x800px, JPG or PNG</div>
                                            <?php endif; ?>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="image-upload-area" onclick="document.getElementById('gallery').click()">
                                            ...
                                            <div class="form-help">Multiple images allowed, JPG or PNG</div>
                                        </div>
                                        <input type="file" id="gallery" name="gallery[]" accept="image/*" multiple style="display: none;">
                                        
                                        <?php if ($is_edit && !empty($tour['gallery'])): ?>
                                            <div class="gallery-preview">
                                                <?php foreach (json_decode($tour['gallery'], true) as $index => $image): ?>
                                                    <div class="gallery-item">
                                                        <img src="../../<?php echo htmlspecialchars($image); ?>" alt="Gallery Image">
                                                        <button type="button" class="remove-btn" onclick="removeGalleryImage(<?php echo $index; ?>)">
                                    ...
                                    <div class="form-group">
                                        <label>Tour Brochure (PDF)</label>
                                        <div class="image-upload-area" onclick="document.getElementById('brochure_pdf').click()">
                                            <?php if ($tour['brochure_pdf'] ?? ''): ?>
                                                <div class="upload-icon">
                                                    <i class="fas fa-file-pdf"></i>
                                                </div>
                                                <div class="upload-text">Current: <?php echo basename($tour['brochure_pdf']); ?></div>
                                                <div class="form-help">Click to replace PDF brochure</div>
                                            <?php else: ?>
                                                <div class="upload-icon">
                                                    <i class="fas fa-file-pdf"></i>
                                                </div>
                                                <div class="upload-text">Click to upload PDF brochure</div>
                                                <div class="form-help">Optional tour brochure for download</div>
                                            <?php endif; ?>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="url" id="video_url" name="video_url" class="form-control" 
                                               ...
                                               value="<?php echo htmlspecialchars($tour['video_url'] ?? ''); ?>">
                                        <div class="form-help">YouTube or Vimeo video URL</div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div style="margin-top: 10px;">
                                            <label class="toggle-switch">
                                                <input type="checkbox" name="featured" value="1" 
                                                       <?php echo ($tour['featured'] ?? 0) ? 'checked' : ''; ?>>
                        ...
                        <div class="form-actions">
                            ...
                            <div class="action-buttons">
                                ...
                                <button type="submit" name="status" value="active" class="btn-save">
                                    <i class="fas fa-check"></i> <?php echo $is_edit ? 'Update Tour' : 'Publish Tour'; ?>
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        document.getElementById('duration_days').addEventListener('input', function() {
            ...
            document.getElementById('duration_nights').value = nights;
        });
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    uploadArea.innerHTML = `
                        <img src="${e.target.result}" alt="Featured Image" class="image-preview">
                        <div class="upload-text">Click to change featured image</div>
                    `;
        ...
        document.getElementById('gallery').addEventListener('change', function(e) {
            ...
            const galleryPreview = document.querySelector('.gallery-preview') || createGalleryPreview();
        ...
        function createGalleryPreview() {
            ...
            document.querySelector('input[name="gallery[]"]').parentElement.appendChild(galleryPreview);
...
document.getElementById('country_id').addEventListener('change', function () {
    ...
    if (countryId) {
        fetch('/admin/api/get_regions.php?country_id=' + countryId)
            .then(response => response.json())
            .then(data => {
                regionSelect.innerHTML = '<option value="">Select Region</option>';
